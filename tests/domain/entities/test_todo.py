...
def test_todo_creation_required_parameters(get_todo_dict: dict, get_User: User):
    ...
    with pendulum.freeze():
        todo = Todo(**get_todo_dict)
        assert (
            todo.date_start == pendulum.now(pendulum.local_timezone()).int_timestamp
        )  # int <- timestamp (int)
        tz = pendulum.local_timezone()
        expected_due = (
            pendulum.from_timestamp(todo.date_start, tz=tz).at(23, 59, 59).int_timestamp
        )
        assert (
            todo.date_due == expected_due
        )  # si due_date n'est pas définie alors elle est égale date_start à 23:59:00
...
def test_todo_creation_missing_required_parameters(
    get_todo_dict: dict, missing_param: str
):
...
def test_todo_creation_due_date_is_set(get_todo_dict: dict):
    todo = Todo(
        **get_todo_dict,
        date_start="2026-01-07 12:12:12",
        date_due="2026-01-17 12:12:12",
    )
...
def test_todo_update_valid_fields():  # except start and due
    ...
    assert todo.description == "Ajout desc."
