...
def test_todo_get_with_children():
    ...
    child1 = Todo(title="Enfant 1", user=user_id, category="test", description="...", parent=parent.uuid)
    child2 = Todo(title="Enfant 2", user=user_id, category="test", description="...", parent=parent.uuid)
    repo.save(child1)
    repo.save(child2)
...
def test_todo_get_fails_for_wrong_user():
    ...
    todo = Todo(uuid=uuid4(), title="Secret", user=owner_id, category="Work", description="...")
    repo.save(todo)
